...
export class OutboundHistoryTable extends CommonTable {
  constructor() {
    super();
    console.log('OutboundHistoryTable constructor called');
    ...
    console.log('Setting table selector: outboundHistoryTable');
  ...
  calculateTotal(row) {
    ...
    const qty = parseFloat(data.INBOUND_QTY) || 0;
  ...
  // 행 추가
  addRow() {
    console.log('OutboundHistoryTable addRow called');
    // const defaultData = this.getDefaultRowData();
    super.addRow();
    console.log('OutboundHistoryTable addRow completed');
  }
